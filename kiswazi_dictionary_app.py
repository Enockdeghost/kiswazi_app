...
# API Configuration
kiswazi_API_URL = "https://od-api-sandbox.kiswazidictionaries.com/api/v2"
APP_ID = "730af7fe"
...
class KiswaziDictionary:
    ...
    def search_word(word):
        ...
        try:
            ...
            endpoints = [
                f"{kiswazi_API_URL}/entries/en-us/{clean_word}",
                f"{kiswazi_API_URL}/entries/en-gb/{clean_word}",
                f"{kiswazi_API_URL}/entries/en/{clean_word}"
            ]
            ...
            for url in endpoints:
                ...
                if response.status_code == 200:
                    try:
                        ...
                        return KiswaziDictionary.parse_kiswazi_response(data, clean_word)
    ...
    @staticmethod
    def parse_kiswazi_response(data, word):
        """Parse kiswazi API response into simplified format"""
        try:
            print("Parsing kiswazi API response...")
            ...
            print(f"Error parsing kiswazi response: {e}")
            import traceback
            traceback.print_exc()
            return None
...
class KiswaziDictionary(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle("Kiswazi Dictionary - kiswazi Powered")
    ...
    def test_api_connection(self):
        ...
        test_word = "test"
        result = KiswaziDictionary.search_word(test_word)
    ...
    def search_word(self):
        ...
        # Try kiswazi API first
        print(f"Searching for: {query}")
        kiswazi_result = KiswaziDictionary.search_word(query)
        
        if kiswazi_result and kiswazi_result['definition'] != "Definition not available":
            print("kiswazi API result found")
            ...
            try:
                ...
                ''', (
                    kiswazi_result['word'],
                    kiswazi_result['definition'],
                    kiswazi_result['part_of_speech'],
                    kiswazi_result['pronunciation'],
                    kiswazi_result['example'],
                    kiswazi_result['synonyms']
                ))
                self.db.conn.commit()
